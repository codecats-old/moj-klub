...
class Notificator_Notificator extends Blinker{	
	...
	private function user_unread_query ()
	{
		...
		//		->where('status', '=', TRUE)
				->where('read', '>=', date('Y-m-d H:i:s', $status['to']));
		}
		else
		{
	...
	private function user_all_unread_query ()
	{
		...
		$unread = $user->request
			->where('read_recipient', '=', NULL)
			->where('read', '>', date('Y-m-d H:i:s', $user->last_login - $this->validity) );
		return $unread;
	}
	//-----------TEAM MESSAGES
	/**
	...
	private function team_unread_query ()
	{
		...
		if ($this->already_shown($user->last_login))
		{
			...
			$unread = $team->request
				->where('date', '>=', date('Y-m-d H:i:s', $status['from']))
				->where('active', '=', TRUE)
				->where('status', '=', NULL)
			;
		}
